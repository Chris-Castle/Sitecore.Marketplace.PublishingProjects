...
namespace Sitecore.Marketplace.PublishingProjects
{
	public static class Helper
	{
		...
		public static List<Item> SearchForItems(ID ProjectId)
		{
            ...
            {
				//var query = context.GetQueryable<ProjectItem>().Where(item => item.ProjectId.Guid == ProjectId.Guid).GetResults();
				var query = context.GetQueryable<ProjectItem>().Where(item => item.Title == "content editor").GetResults();
				//var query = context.GetQueryable<ProjectItem>().Where(item => item.ProjectSM != String.Empty).GetResults(); 
				//var query = context.GetQueryable<ProjectItem>().Where(item => item.Fields["project"] != "sadf").GetResults();
			...
			foreach (Item i in item.Axes.GetDescendants())
			{
				if (i["Project"] != String.Empty)
				{
					if (new ID(i["Project"]) == ProjectId)
					results.Add(i);
				}
			}
